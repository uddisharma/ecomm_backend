...
const bulkInsertCategory = async (req, res) => {
  try {
    ...
    createdCategorys = {
      count: createdCategorys ? createdCategorys.length : 0,
    };
...
const getAllCategories = async (req, res) => {
  try {
    ...
    // Populate parentCategoryId recursively for each category
...
async function populateParentCategories1(category) {
  if (
    category.parentCategoryId &&
    typeof category.parentCategoryId === "object"
  ) {
...
// Controller to find a single category by ID and populate parentCategoryId recursively
const getCategoryById = async (req, res) => {
  try {
    const categoryId = req.params.id; // Assuming the category ID is provided in the request parameters
    ...
    // Populate parentCategoryId recursively for the category
    await populateParentCategories1(category);
...
const updateCategory = async (req, res) => {
  try {
    ...
    let updatedCategory = await dbService.updateOne(
      Category,
      query,
      dataToUpdate
    );
...
const bulkUpdateCategory = async (req, res) => {
  try {
    ...
    let updatedCategory = await dbService.updateMany(
      Category,
      filter,
      dataToUpdate
    );
...
const softDeleteCategory = async (req, res) => {
  try {
    ...
    let updatedCategory = await deleteDependentService.softDeleteCategory(
      query,
      updateBody
    );
    if (!updatedCategory) {

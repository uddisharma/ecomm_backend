...
const bulkInsertWalletTransaction = async (req, res) => {
  try {
    ...
    let createdWalletTransactions = await dbService.create(
      WalletTransaction,
      dataToCreate
    );
    createdWalletTransactions = {
      count: createdWalletTransactions ? createdWalletTransactions.length : 0,
    };
    return res.success({
      data: { count: createdWalletTransactions.count || 0 },
    });
  } catch (error) {
    return res.internalServerError({ message: error.message });
  }
...
const deleteManyWalletTransaction = async (req, res) => {
  try {
    ...
    const deletedWalletTransaction = await dbService.deleteMany(
      WalletTransaction,
      query
    );
    if (!deletedWalletTransaction) {
